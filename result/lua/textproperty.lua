...
local bpmmin = main_state.number(91)
local bpmmax = main_state.number(90)
local songmin = main_state.number(1163)
local songsec = main_state.number(1164)
...
local star_str = "☆"
if main_state.option(155) then
	star_str = "★"
end

local difflvl_str = diff_str .. " " .. star_str .. main_state.number(96)

local gauge = main_state.number(107)
local gaugedec = main_state.number(407)
local gaugetext_str = gauge .. "." .. gaugedec .. "%"
...
module.text = {
	...
	{id = "level", font = 0, size = 20, align = 0, overflow = 1, constantText = level_str},
